# calculate_dist() / calculate_dendrogram()

test_that("calculate_dist returns a dist object with one entry per sample pair", {
  set.seed(1)
  mat <- matrix(rpois(50 * 5, lambda = 40) + 1, nrow = 50)
  colnames(mat) <- paste0("s", 1:5)
  rownames(mat) <- paste0("g", 1:50)

  d <- calculate_dist(mat, cor_method = "pearson")

  expect_s3_class(d, "dist")
  expect_length(d, choose(5, 2))
})

test_that("calculate_dendrogram produces an hclust tree with one merge per sample gap", {
  set.seed(1)
  mat <- matrix(rpois(50 * 5, lambda = 40) + 1, nrow = 50)
  colnames(mat) <- paste0("s", 1:5)
  rownames(mat) <- paste0("g", 1:50)

  hcd <- calculate_dendrogram(mat, cor_method = "pearson", cluster_method = "average")
...
test_that("interactive_clustering_dendrogram puts the branch tree in trace 0 and hides toggled-off groups", {
  set.seed(4)
  grps <- c("A", "B", "C")
  ...
  built <- plotly::plotly_build(interactive_clustering_dendrogram(mat, experiment, colorby = "group", hidden_groups = "B", source = "test"))
  traces <- built$x$data
...
test_that("interactive_clustering_dendrogram falls back to a placeholder when fewer than two groups remain visible", {
  set.seed(5)
  ...
  built <- suppressWarnings(plotly::plotly_build(interactive_clustering_dendrogram(mat, experiment, colorby = "group", hidden_groups = c("A", "B"), source = "test2")))
