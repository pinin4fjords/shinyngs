...
test_that("interactive_barcodeplot draws a rug-tick trace and an enrichment worm curve", {
  set.seed(1)
  gene_ids <- paste0("gene", 1:100)
  fold_changes <- rnorm(100)
  set_gene_ids <- sample(gene_ids, 15)

  built <- plotly::plotly_build(interactive_barcodeplot(fold_changes, gene_ids, set_gene_ids))
...
test_that("interactive_barcodeplot handles no gene set members found in the ranked list", {
  gene_ids <- paste0("gene", 1:20)
  fold_changes <- rnorm(20)

  p <- suppressWarnings(interactive_barcodeplot(fold_changes, gene_ids, set_gene_ids = "not_present"))
...
test_that("interactive_barcodeplot drops NA fold changes before ranking", {
...
test_that("interactive_barcodeplot converts newlines in the title to <br> tags", {
  gene_ids <- paste0("gene", 1:10)
  fold_changes <- rnorm(10)

  built <- plotly::plotly_build(interactive_barcodeplot(fold_changes, gene_ids, set_gene_ids = gene_ids[1], plot_title = "line one\nline two"))
