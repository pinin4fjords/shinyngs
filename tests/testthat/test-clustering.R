...
test_that("run_clustering partitions rows into the requested number of clusters", {
  set.seed(1)
  mat <- matrix(rnorm(60), nrow = 10, ncol = 6)
  rownames(mat) <- paste0("gene", 1:10)
  colnames(mat) <- paste0("sample", 1:6)

  clusters <- run_clustering(mat, 3)

  expect_equal(length(unique(clusters$clustering)), 3)
  expect_equal(length(clusters$clustering), nrow(mat))
})

test_that("run_clustering raises an informative error when more clusters are requested than features", {
  mat <- matrix(rnorm(24), nrow = 4, ncol = 6)
  rownames(mat) <- paste0("gene", 1:4)
  colnames(mat) <- paste0("sample", 1:6)

  expect_error(run_clustering(mat, 6), "must be between 1 and the number of available features")
})

test_that("run_clustering raises an informative error with fewer than 2 features", {
  mat <- matrix(rnorm(6), nrow = 1, ncol = 6)
  rownames(mat) <- "gene1"
  colnames(mat) <- paste0("sample", 1:6)

  expect_error(run_clustering(mat, 1), "at least 2 features")
})
...
test_that("interactive_cluster_profiles draws a main line plus a shaded error band per cluster under 'filled_line'", {
  mbc <- make_cluster_fixture()

  built <- plotly::plotly_build(interactive_cluster_profiles(mbc, cluster_display = "filled_line", average_type = "mean", limits = "sd"))
...
test_that("interactive_cluster_profiles draws one line-with-error-bars trace per cluster under 'error_bars'", {
  mbc <- make_cluster_fixture()

  built <- plotly::plotly_build(interactive_cluster_profiles(mbc, cluster_display = "error_bars", average_type = "mean", limits = "sd"))

  expect_length(built$x$data, 2)

  summarised <- summary_se(melt_matrix(mbc[["3"]]), measurevar = "value", groupvars = "Var2")
...
test_that("interactive_cluster_profiles derives the median-specific column without requiring a pre-prefixed limits argument", {
  mbc <- make_cluster_fixture()

  # limits = "sd" resolves against the median-specific column even without the caller
  # pre-prefixing it - a caller shouldn't need to know summary_se()'s internal column-naming
  # convention. median.sd comes from a bootstrap (bootstrap_median()), so both calls need the
  # same seed to compare.
  set.seed(42)
  built <- plotly::plotly_build(interactive_cluster_profiles(mbc, cluster_display = "error_bars", average_type = "median", limits = "sd"))

  set.seed(42)
  summarised <- summary_se(melt_matrix(mbc[["3"]]), measurevar = "value", groupvars = "Var2", add_medians = TRUE)
  cluster3 <- built$x$data[[which(vapply(built$x$data, function(t) identical(as.character(t$name), "Cluster 3"), logical(1)))]]
...
test_that("interactive_cluster_profiles indexes colors by cluster number, not by list position", {
  mbc <- make_cluster_fixture()
  colors <- paste0("color", 1:10)

  built <- plotly::plotly_build(interactive_cluster_profiles(mbc, cluster_display = "error_bars", colors = colors))
...
test_that("interactive_cluster_profiles subsamples large clusters to max_sample_lines under 'sample_lines'", {
  set.seed(1)
  big_cluster <- matrix(rnorm(50 * 4), nrow = 50, dimnames = list(paste0("g", 1:50), paste0("s", 1:4)))

  built <- plotly::plotly_build(interactive_cluster_profiles(list(`1` = big_cluster), cluster_display = "sample_lines", max_sample_lines = 10))
...
test_that("interactive_cluster_profiles uses a precomputed summarised_matrices_by_cluster instead of recomputing summary_se()", {
  mbc <- make_cluster_fixture()
  ...
  fake_summary <- lapply(mbc, function(m) {
    s <- summary_se(melt_matrix(m), measurevar = "value", groupvars = "Var2")
    s$mean <- 999
    s
  })

  built <- plotly::plotly_build(interactive_cluster_profiles(
    mbc,
...
test_that("mad_score flags a sample decorrelated from its group as an outlier", {
  set.seed(1)
  n_genes <- 50
  base <- rnorm(n_genes)
  ...
  result <- mad_score(mat, sample_sheet, groupby = "group")

  expect_true(result["s5", "outlier"])
  expect_false(any(result[paste0("s", 1:4), "outlier"]))
})

test_that("mad_score warns and returns NULL when no group has enough replicates", {
  ...
  expect_warning(result <- mad_score(mat, sample_sheet, groupby = "group"), "low replication")
  expect_null(result)
})

test_that("mad_score drops samples belonging to under-replicated groups before scoring", {
  set.seed(2)
  n_genes <- 30
  base <- rnorm(n_genes)
