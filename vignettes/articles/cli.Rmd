...
Mandatory arguments: `--sample_metadata`, `--feature_metadata`,
`--assay_files`, and `--output_directory`. To include differential views,
provide `--contrast_file` and `--differential_results` together.
...
| `-y`, `--contrast_stats_assay` | integer; `NULL` | 1-based index indicating which `--assay_files` element the contrast statistics relate to (usually a normalised matrix). Defaults to the last assay. |
...
| Flag | Type / default | Description |
|------|----------------|-------------|
| `--heatmap_row_height_px` | integer; `12` | Pixel height per row in heatmaps with row labels shown (e.g. the expression heatmap). |
| `--heatmap_row_height_wide_px` | integer; `20` | Pixel height per row in heatmaps with fewer, wider rows (the samples and PCA-vs-experiment heatmaps). |
| `--heatmap_row_height_no_labels_px` | integer; `2` | Pixel height per row when row labels are hidden. |
| `--heatmap_max_height_no_labels_px` | integer; `1200` | Ceiling on the default heatmap plot height when row labels are hidden. |
| `--heatmap_annotation_row_height_px` | integer; `26` | Pixel height per column annotation color bar row. |
| `--heatmap_dendrogram_height_px` | integer; `150` | Pixel height of the column dendrogram row shown above a heatmap when columns are clustered. |
| `--heatmap_subplot_gap_px` | integer; `4` | Visual gap, in pixels, between stacked heatmap elements (annotation bars, column dendrogram, heatmap body). |

These map directly onto `heatmap_layout_options()`'s arguments and are baked
into the generated `app.R` as a `prepare_app("rnaseq", esel, heatmap_layout =
heatmap_layout_options(...))` call, so they take effect every time that app
bundle is run.
...
internally (as in `eselist_from_config()`). It checks that sample metadata,
