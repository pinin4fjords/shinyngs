...
- `ggplot_boxplot()` draws a static ggplot2 boxplot.
- `plotly_boxplot()` draws the interactive plotly version.
- `plotly_quartiles()` / `plotly_densityplot()` are the alternative renderings.
...
`ggplot_*` / `plotly_*` function and have the module call it.
...
`plotly_myplot()` (and/or `ggplot_myplot()`) that takes plain arguments. Give it
...
**4. Wire it into an app.** Because `prepareApp()` and `simpleApp()` locate a
