...
clusteringInput <- function(id, eselist) {
    ...
    cluster_filters <- list(selectInput(ns("cluster_number"), label = "Number of clusters", choices = 1:12, selected = 6), 
        ...
        colormakerInput(ns("clustering")), selectInput(ns("average_type"), label = "Average type", choices = c("mean", 
            "median"), selected = "mean"), selectInput(ns("limits"), label = "Limits show", choices = c(`Standard deviation` = "sd", 
...
clustering <- function(input, output, session, eselist, ncolors) {
    ...
    # getClusterColors <- reactive({ number_of_clusters <- getClusterNumber()
