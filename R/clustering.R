...
clusteringInput <- function(id, eselist) {
  ...
  expression_filters <-
    selectmatrixInput(ns("clustering"), eselist)
  ...
  cluster_filters <- list(
    ...
    selectInput(
      ...
      choices = cluster_displays,
      selected = 'filled_line'
    ),
    selectInput(
      ns('average_type'),
      label = 'Average type',
    ...
    selectInput(
      ns('limits'),
      label = 'Limits show',
      choices = c('Standard deviation' = 'sd', 'Standard error' = 'se', '95% confidence interval' = 'ci'),
      selected = 'sd'
    )
  )
  ...
  fieldSets(
    ns("fieldset"),
    list(
      clustering = cluster_filters,
      expression = expression_filters,
      export = simpletableInput(ns("geneClusteringTable"))
    )
  )
}
...
clusteringOutput <- function(id) {
  ns <- NS(id)
  list(
    modalInput(ns("clustering"), "help", "help"),
    modalOutput(
      ns("clustering"),
      "Feature-wise clustering",
      includeMarkdown(
        system.file("inlinehelp", "clustering.md",
                    package = packageName())
      )
    ),
...
clustering <- function(input, output, session, eselist) {
  ...
  # Call the selectmatrix module to get the expression matrix - no need for a 
  # gene selection
  ...
  getClusterNumber <- reactive({
    validate(need(
      !is.null(input$cluster_number),
      'waiting for cluster number'
    ))
  ...
  getClusterDisplay <- reactive({
    validate(need(
      !is.null(input$cluster_display),
      'waiting for cluster display'
    ))
  ...
  # Generate summary statistics from the matrix for each cluster. This is what's
  # actually plotted if cluster display is set to something other than 'sample
  # lines'. Uses summarySE() to derive mean, median, standard error, standard
  ...
  getSummarisedMatricesByCluster <- reactive({
    ...
    withProgress(message = "Calculating summary statistics for the clusters", value = 0, {
      lapply(matrices_by_cluster, function(mbc) {
        summarySE(reshape2::melt(as.matrix(mbc)),
                  measurevar = 'value',
                  groupvars = 'Var2',
  ...
  # The business end of the cluster plotting. Use the above parameters and 
  # processing to produce one plot object for each cluster of matrix rows.
  ...
  makeClusterPlots <- reactive({
    ...
    if (cluster_display == 'sample_lines') {
      ...
      plots <- lapply(names(summarised_matrices_by_cluster), function(c) {
        ...
        plot_ly(
          ...
          y = ~ x[, average_type],
          name = paste('Cluster', c)
        ) %>% add_lines(error_y = ~ list(name = paste('Cluster', c), color = cluster_color, type = 'array', array = x[,limits]),
                        line = list(color = cluster_color))
      })
    } else{
      plots <- lapply(names(matrices_by_cluster), function(c) {
    ...
    lapply(plots, function(p) {
      p %>% layout(
        ...
        yaxis = list(title = 'scaled<br />expression'),
        margin = list(b = 150)
      )
    })
  })
  
  # Take individual cluster plots and display them together using
  # subplot.
  ...
  makeMatrixWithClusters <- reactive({
    ...
    inmatrix$cluster <-
      clusters$clustering[match(rownames(inmatrix), names(clusters$clustering))]
    inmatrix <-
      inmatrix[, c('cluster', colnames(inmatrix)[colnames(inmatrix) != 'cluster'])]
  ...
  callModule(
    simpletable,
    "geneClusteringTable",
    downloadMatrix = makeMatrixWithClusters,
    displayMatrix = makeLinkedMatrixWithClusters,
    filter = 'top',
