...
clustering <- function(input, output, session, eselist, ncolors) {
    ...
    # Generate summary statistics from the matrix for each cluster. This is what's actually plotted if cluster display is set to something other
    # than 'sample lines'. Uses summarySE() to derive mean, median, standard error, standard deviation and 95% confidence intervals.
    ...
    # The business end of the cluster plotting. Use the above parameters and processing to produce one plot object for each cluster of matrix
    # rows.
        ...
        if (cluster_display == "sample_lines") {
            ...
            plots <- lapply(names(matrices_by_cluster), function(c) {
                ...
                plot_ly(x, name = paste("Cluster", c)) %>% add_trace(x = ~Var2, y = x[, average_type], type = "scatter", mode = "lines", line = list(color = cluster_color), 
                  showlegend = TRUE) %>% add_trace(x = ~Var2, y = ~upper, type = "scatter", mode = "lines", line = list(color = "transparent"), 
                  showlegend = FALSE, name = paste(average_type, toupper(limits), sep = " + ")) %>% add_trace(x = ~Var2, y = ~lower, type = "scatter", 
                  mode = "lines", fill = "tonexty", fillcolor = "rgba(0,100,80,0.2)", line = list(color = "transparent"), showlegend = FALSE, 
