...
prepareApp <- function(type, ses, title = "ShinyNGS application", ui_only = FALSE, ...) {
    ...
    # colnames(colData(se))[unlist(lapply(names(colData(se)), function(var) is.factor(colData(se)[[var]])))] } if (!  'default_groupvar' %in% colnames(metadata(se))){
    # metadata(se)$default_groupvar <- metadata(se)$group_vars[1] } se })
...
# simpletableLayout <- function(se, params) { sidebarLayout(sidebarPanel(simpletableInput('simpletable', description = 'These are the samples involved in this
# study, and their associated variables.  Contrasts for differential expression are built from these variables'), width = 3),
