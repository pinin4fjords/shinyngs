...
prepareApp <- function(type, ses, title = "ShinyNGS application", ui_only = FALSE, ...) {
    ...
    # ses <- lapply(ses, function(se){ if (! 'group_vars' %in% colnames(metadata(se))){ metadata(se)$group_vars <- colnames(colData(se))[unlist(lapply(names(colData(se)),
    # function(var) is.factor(colData(se)[[var]])))] } if (!  'default_groupvar' %in% colnames(metadata(se))){ metadata(se)$default_groupvar <- metadata(se)$group_vars[1] } se })
    ...
    # if (type == 'simpletable') { ui <- fluidPage(shinyjs::useShinyjs(), navbarPage(id = 'pages', title = 'A simple table page:', tabPanel('Home', simpletableLayout(se,
    # params)))) server <- function(input, output, session) { callModule(simpletable, 'simpletable', data.frame(colData(se))) } }
...
simpleApp <- function(ses, module = NULL, title = NULL, ui_only = FALSE, ...) {
    ...
    if (!is.null(module)) {
        ...
        ui <- fluidPage(includeCSS(cssfile), theme = shinythemes::shinytheme("cosmo"), shinyjs::useShinyjs(), navbarPage(id = "pages", title = title, windowTitle = title, tabPanel(prettifyVariablename(module), 
            sidebarLayout(sidebarPanel(inputFunc(module, ses, ...), width = 3), mainPanel(outputFunc(module, ...), width = 9)))))
...
# simpletableLayout <- function(se, params) { sidebarLayout(sidebarPanel(simpletableInput('simpletable', description = 'These are the samples involved in this study, and their
# associated variables.  Contrasts for differential expression are built from these variables'), width = 3), mainPanel(simpletableOutput('simpletable', tabletitle =
# 'Experimental variables'), width = 9)) } 
