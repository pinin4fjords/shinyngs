...
foldchangeplot <- function(input, output, session, eselist) {
    ...
    plotLines <- reactive({
        ...
        lines <- data.frame(name = c(rep("No change", 2), rep(paste0(fcMin(), "-fold down"), 2), rep(paste0(fcMin(), "-fold up"), 2)), x = c(min, max, 
            min, max, min, max), y = c(c(min, max), (min - log2(fcMin())), (max - log2(fcMin())), (min + log2(fcMin())), (max + log2(fcMin()))), stringsAsFactors = FALSE)
