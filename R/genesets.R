...
geneset <- function(input, output, session, annotation, entrezgenefield, genefield, geneset_files) {
    ...
    output$geneSets <- renderUI({
        ...
        gene_set_names <- structure(paste(unlist(lapply(1:length(gene_sets), function(x) paste(x, 1:length(gene_sets[[x]]), sep = "-")))), names = unlist(lapply(names(gene_sets), function(settype) paste0(prettifyVariablename(names(gene_sets[[settype]])), 
            " (", settype, ")"))))
