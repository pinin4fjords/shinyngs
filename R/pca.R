...
pcaInput <- function(id, ses, principal_components = structure(1:10, names = paste0("PC", 1:10))) {
    ...
    pca_filters <- list(selectInput(ns("xAxisComponent"), "x axis component", principal_components, selected = 1), selectInput(ns("yAxisComponent"), "y axis component", principal_components, selected = 2), selectInput(ns("zAxisComponent"), 
        "z axis component", principal_components, selected = 3), uiOutput(ns("samplePCAColorBy")))
...
plotlyPCA <- function(pcavals, pcX, pcY, pcZ, pcameta, colorby = NULL, title = "foo") {
    ...
    p <- layout(p, xaxis = list(title = colnames(plotdata)[pcX]), yaxis = list(title = colnames(plotdata)[pcY]), zaxis = list(title = colnames(plotdata)[pcZ]), scene = list(xaxis = list(title = colnames(plotdata)[pcX]), yaxis = list(title = colnames(plotdata)[pcY]), 
        zaxis = list(title = colnames(plotdata)[pcZ])), margin = list(l = 0, r = 0, t = 50, b = 0), legend = list(y = 0.8), title = title)
