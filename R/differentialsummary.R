...
#'
#' @return Controls for experiment, assay, differential filters, and export.
#' @keywords shiny
differentialsummaryInput <- function(id, eselist) {
  ns <- NS(id)

  fieldSets(ns("fieldset"), list(
    expression = selectmatrixInput(ns("summary"), eselist),
    differential_filters = contrastsInput(ns("summary"), select_summary_type = TRUE),
    export = simpletableInput(ns("table"), tabletitle = "Differential counts")
  ))
}
...
#' @param id Module namespace.
#'
#' @return Outputs for the filter description, plot, and backing table.
#' @keywords shiny
differentialsummaryOutput <- function(id) {
  ns <- NS(id)

  moduleMain(
    "Differential results across contrasts",
    uiOutput(ns("differential_parameters")),
    uiOutput(ns("plot_ui")),
    h4("Differential counts"),
...
differentialsummary <- function(id, eselist) {
  moduleServer(id, function(input, output, session) {
    ...
    selectmatrix_reactives <- selectmatrix(
      "summary", eselist,
      var_n = 50, select_samples = FALSE, select_genes = FALSE,
      provide_all_genes = TRUE, select_meta = FALSE
    )
    contrast_reactives <- contrasts(
      "summary", eselist = eselist, selectmatrix_reactives = selectmatrix_reactives,
    ...
    output$differential_parameters <- renderUI({
      HTML(contrast_reactives$getQueryStrings()[1])
    })
    ...
    getDifferentialSummaryPlot <- reactive({
      summary <- getDifferentialSummary()
      interactive_differential_summary(summary)
