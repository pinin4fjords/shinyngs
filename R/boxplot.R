...
ggplot_boxplot <- function(plotmatrix, experiment, colorby = NULL, palette = NULL, expressiontype = "expression", whisker_distance = 1.5) {
    ...
    p <- p + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, size = rel(1.5)), axis.title.x = element_blank(), 
        ...
        title = element_text(size = rel(1.3))) + ylab(splitStringToFixedwidthLines(paste0("log2(", expressiontype, ")"), 15))
...
plotly_quartiles <- function(matrix, ese, expressiontype = "expression", whisker_distance = 1.5) {
    ...
    # These lines to force plotly to use and display sample IDs as strings. For some reason character strings of numeric things
    # get converted back
    ...
    plot_ly(data.frame(quantiles), mode = "markers") %>% add_trace(x = outliers$x, y = outliers$y, name = "outliers", marker = list(color = "black"), 
        ...
        whisker_distance, ")")) %>% add_lines(x = samples, y = quantiles["75%", samples], line = list(dash = "dash", color = "black"), 
        name = "75%") %>% add_lines(x = samples, y = quantiles["50%", samples], line = list(dash = "solid", color = "black"), 
        name = "median") %>% add_lines(x = samples, y = quantiles["25%", samples], line = list(dash = "longdash", color = "black"), 
        name = "25%") %>% add_lines(x = samples, y = quantiles["25%", ] - ((quantiles["75%", ] - quantiles["25%", ]) * whisker_distance), 
        line = list(width = 1, color = "grey", dash = "longdash"), name = paste0("25%<br />- (IQR * ", whisker_distance, ")")) %>% 
        layout(xaxis = list(title = NULL, categoryarray = samples, categoryorder = "array"), yaxis = list(title = paste0("log2(", 
