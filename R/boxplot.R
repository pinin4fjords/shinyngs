...
plotly_quartiles <- function(matrix, ese, expressiontype = "expression", whisker_distance = 1.5) {
    ...
    plot_ly(data.frame(quantiles), mode = 'markers') %>% add_trace(x = outliers$x, y = outliers$y, name = "outliers", marker = list(color = "black"), hoverinfo = "text", 
        ...
        line = list(dash = "solid", color = "black"), name = "median") %>% add_lines(x = samples, y = quantiles["25%", samples], line = list(dash = "longdash", 
        color = "black"), name = "25%") %>% add_lines(x = samples, y = quantiles["25%", ] - ((quantiles["75%", ] - quantiles["25%", ]) * whisker_distance), 
        line = list(width = 1, color = "grey", dash = "longdash"), name = paste0("25%<br />- (IQR * ", whisker_distance, ")")) %>% layout(xaxis = list(title = NULL, 
        categoryarray = samples, categoryorder = "array"), yaxis = list(title = paste0("log2(", expressiontype, ")")), margin = list(b = 150), hovermode = "closest", 
        title = NULL)
