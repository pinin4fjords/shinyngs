...
ggplot_boxplot <- function(plotmatrix, experiment, colorby = NULL, expressiontype = "Normalised counts per million") {
    ...
    p <- p + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, size = rel(1.8)), axis.title.x = element_blank(), legend.position = "bottom", axis.text.y = element_text(size = rel(1.8)), 
        legend.text = element_text(size = rel(1.8)), title = element_text(size = rel(2))) + ylab(splitStringToFixedwidthLines(paste0("log2(", expressiontype, ")"), 15))
