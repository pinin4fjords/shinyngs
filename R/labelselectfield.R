...
labelselectfield <- function(input, output, session, eselist, getExperiment = NULL, labels_from_all_experiments = FALSE, url_field = 'label'){
  ...
  # This module will normally be initialised with a reactive that returns the
  # currently selected experiment, whose metadata will be used for gene 
  # symbols etc. But if that reactive is not present, we can use values from 
  # ALL experiments. In the latter case the field will be more static, in the 
  # former it will depend on the value of any experiment-selecting field.
