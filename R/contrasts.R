...
contrastsInput <- function(id){
  ...
  list(
    uiOutput(ns('contrasts')),
    summarisematrixInput(ns('contrasts'), allow_none = FALSE)
  )
}
...
contrasts <- function(input, output, session, getExperiment, selectMatrix, getAssay, multiple = TRUE){
  ...
  output$contrasts <- renderUI({
    ...
    if (! is.null(contrasts)){
      checkboxGroupInput(ns("contrasts"), "Contrast(s):", contrasts, selected = contrasts) 
    }
  })
  ...
  # Generate the summary statistic (probably mean) for column groups as defined 
  # by the possible contrasts. Other functions can then pick from this output
  # and calculate fold changes etc.
  ...
  # Main function for returning the table of contrast information. Means, fold
  # changes calculated on the fly, p/q values must be supplied in a 'tests' 
  # slot of the metadata.
  
  contrastsTable <- reactive({
    matrix <- selectMatrix()
    ...
    withProgress(message = "Calculating summary data", value = 0, {
      ...
      contrast_table <- do.call(rbind, lapply(as.numeric(input$contrasts), function(c){
        c <- as.numeric(c)
        ...
        ct <- data.frame(cont[1], cont[2], smry1, cont[3], smry2, foldChange(smry1, smry2))
        names(ct) <- c('Variable', 'Condition 1', 'Average 1', 'Condition 2', 'Average 2', 'Fold change')
        ...
        if ('tests' %in% names(metadata(se)) && getAssay() %in% names(metadata(se)$tests)){
          ...
          ct[['p value']] <- pvals[match(rownames(ct), rownames(pvals)), c]
          ct[['q value']] <- qvals[match(rownames(ct), rownames(qvals)), c]
  ...
  labelledContrastsTable <- reactive({  
    ct <- contrastsTable()
    ct$Variable <- prettifyVariablename(ct$Variable)
  ...
  list(contrastsTable = contrastsTable, labelledContrastsTable = labelledContrastsTable, linkedLabelledContrastsTable = linkedLabelledContrastsTable)
}
