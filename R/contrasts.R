...
contrastsInput <- function(id, allow_filtering = TRUE, summarise = TRUE, dynamic_filters = FALSE, select_summary_type = FALSE) {
    ...
    # Contrasts filters added by the observeEvent() in the server function. If no dynamic filters are to be provided, then all we need to do is provide a placeholder
    # and a single set of filters will be provided when the page loads. If dynamic filters ARE to be provided, then some other buttons etc are required.
    ...
    if (dynamic_filters) {
        contrast_filters <- c(list(hiddenInput(ns("dynamic"), 1)), list(helpText("Build up a complex query by adding filters below"), hr()), contrast_filters, list(hr(), 
            uiOutput(ns("combine_operator")), actionButton(ns("insertBtn"), "Add"), HTML("&nbsp;"), actionButton(ns("removeBtn"), "Remove")))
    } else {
...
contrasts <- function(input, output, session, eselist, selectmatrix_reactives = list(), multiple = FALSE, select_all_contrasts = FALSE, show_controls = TRUE, default_min_foldchange = 2) {
    ...
    observeEvent({
        ...
        # filterset_values. When this is referenced (by e.g. getFoldChange()), the dependency chain is established such that outputs are refreshed when the dynamically
        # added fields are altered.
    ...
    ########################################################################### Generate summaries and contrast stats for all contrasts and all rows.  Then the data is handy for subsetting by other functions.  NOTE: this uses ALL rows in
    ########################################################################### the input ExploratorySummarizedExperiment.  The rows in the matrix returned by selectMatrix() will but a subset, but any modifications to the rows used will not
    ########################################################################### necessitate a re-calculation of these basic stats.
    ...
    # ExploratorySummarizedExperiment. Make a summary table for every contrast. This data can then be re-used when processing filter sets.
    ...
    ########################################################################### Subsetting using the rows in the input matrix. This does NOT involve the filters from this module, but simply subsets the base data to the rows pertinent to the
    ########################################################################### input matrix.
