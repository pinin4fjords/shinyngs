...
labelMatrix <- function(matrix, ese, idcol = NULL, metafields = c()) {
    ...
    # if (length(ese@labelfield) > 0) { labelfield <- ese@labelfield matrix[[labelfield]] <- convertIds(matrix[[idfield]], ese, labelfield) matrix <-
    # matrix[, c(idfield, labelfield, datacolnames), drop = FALSE]
    
    # colnames(matrix)[colnames(matrix) == labelfield] <- prettifyVariablename(labelfield) } else { matrix <- matrix[, c(idfield, datacolnames), drop =
    # FALSE] }
