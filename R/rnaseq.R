...
rnaseqInput <- function(id, ses, title = "") {
    ...
    navbar_menus <- list(id = ns("rnaseq"), title = paste0("RNA-seq explorer: ", title), windowTitle = title, tabPanel("Home", sidebarLayout(sidebarPanel(width = 3), mainPanel(width = 9))), navbarMenu("Sample data", 
        tabPanel("Experiment", sidebarLayout(sidebarPanel(experimenttableInput(ns("experimenttable"), ses), width = 3), mainPanel(experimenttableOutput(ns("experimenttable")), width = 9)))), navbarMenu("QC/ exploratory", 
        tabPanel("Boxplots", sidebarLayout(sidebarPanel(boxplotInput(ns("boxplot"), ses), width = 3), mainPanel(boxplotOutput(ns("boxplot")), width = 9))), tabPanel("PCA", sidebarLayout(sidebarPanel(pcaInput(ns("pca"), 
            ses), width = 3), mainPanel(pcaOutput(ns("pca")), width = 9))), tabPanel("Clustering dendrogram", sidebarLayout(sidebarPanel(dendroInput(ns("dendro"), ses), width = 3), mainPanel(dendroOutput(ns("dendro")), 
    ...
    if (any(unlist(lapply(ses, function(se) "contrasts" %in% names(metadata(se)))))) {
        navbar_menus <- pushToList(navbar_menus, navbarMenu("Differential", tabPanel("Tables", sidebarLayout(sidebarPanel(differentialtableInput(ns("differential"), ses), width = 3), mainPanel(differentialtableOutput(ns("differential")), 
            width = 9)))))
    }
    
    # Add the gene info plots
    
    navbar_menus <- pushToList(navbar_menus, tabPanel("Gene info", value = "geneinfo", sidebarLayout(sidebarPanel(geneInput(ns("gene"), ses), width = 3), mainPanel(geneOutput(ns("gene")), width = 9))))
