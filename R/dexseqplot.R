...
dexseqplotInput <- function(id, eselist) {
    ...
    field_sets <- list(gene = list(labelselectfieldInput(ns("genesymbol"))), differential_exon_usage = c(table_fields$differential_exon_usage, 
        list(numericInput(ns("deuQvalPlotMax"), "Maximum false discovery rate", value = 0.1), checkboxInput(ns("deuExpression"), 
            ...
            "Show transcripts?", value = TRUE))), export = list(p(plotdownloadInput(ns("deuPlot"), "DEU plot")), br(), 
...
dexseqplotOutput <- function(id, eselist) {
    ...
    list(modalInput(ns("dexseqplot"), "help", "help"), modalOutput(ns("dexseqplot"), "Differential exon usage plot", includeMarkdown(system.file("inlinehelp", 
        "dexseqplot.md", package = packageName()))), h4("Gene-wise differential exon usage"), plotOutput(ns("deuPlot"), 
        height = 620), dexseqtableOutput(ns("deuPlotTable")))
}
...
dexseqplot <- function(input, output, session, eselist) {
    ...
    callModule(plotdownload, "deuPlot", makePlot = makeDEUPlotForDownload, filename = "deuplot.png", plotHeight = 800, 
        plotWidth = 1200)
