...
dexseqplotInput <- function(id, eselist) {
    ...
    field_sets <- list(gene = list(labelselectfieldInput(ns("genesymbol"))), differential_exon_usage = c(table_fields$differential_exon_usage, list(numericInput(ns("deuQvalPlotMax"), "Maximum false discovery rate", 
        value = 0.1), checkboxInput(ns("deuExpression"), "Show expression plot?", value = TRUE), checkboxInput(ns("deuSplicing"), "Show exon usage (adjusted for gene expression)", value = TRUE), 
        checkboxInput(ns("deuNorcounts"), "Show normalised counts plot?", value = FALSE), checkboxInput(ns("deuDisplayTranscripts"), "Show transcripts?", value = TRUE))), export = list(p(plotdownloadInput(ns("deuPlot"), 
        "DEU plot")), br(), p(table_fields$export)))
...
dexseqplotOutput <- function(id, eselist) {
    ...
    list(modalInput(ns("dexseqplot"), "help", "help"), modalOutput(ns("dexseqplot"), "Differential exon usage plot", includeMarkdown(system.file("inlinehelp", "dexseqplot.md", package = packageName()))), 
...
dexseqplot <- function(input, output, session, eselist) {
    ...
    unpack.list(callModule(dexseqtable, "deuPlotTable", eselist = eselist, allow_filtering = FALSE, getDEUGeneID = getDEUGeneID, show_controls = FALSE, page_length = 50, link_to_deu_plot = FALSE))
    ...
    getDEUGeneID <- reactive({
        ...
        # Sometimes an exon can be part of multiple Ensembl gene records. The group ID will then be a composite like 'ENSRNOG00000051235+ENSRNOG00000051158+ENSRNOG00000000419'
    ...
    output$deuPlot <- renderPlot({
        ...
        withProgress(message = "Making differential exon usage plot", value = 0, {
            ...
            DEXSeq::plotDEXSeq(dexseq_result, geneID = gene_id, FDR = input$deuQvalPlotMax, fitExpToVar = selected_contrast[[1]][1], norCounts = input$deuNorcounts, splicing = input$deuSplicing, 
                displayTranscripts = input$deuDisplayTranscripts, expression = input$deuExpression, names = TRUE, legend = TRUE, cex.axis = 1.2, cex = 1.3, lwd = 2)
        })
    }, height = 600)
