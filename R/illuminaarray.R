...
illuminaarrayInput <- function(id, eselist) {
    ...
    exploratory_menu <- list("QC/ exploratory", tabPanel("Quartile plots", sidebarLayout(sidebarPanel(boxplotInput(ns("boxplot"), eselist), width = 3), 
        mainPanel(boxplotOutput(ns("boxplot")), width = 9)), icon = icon("bar-chart-o")), tabPanel("PCA", sidebarLayout(sidebarPanel(pcaInput(ns("pca"), 
        ...
        tabPanel("Clustering Heatmap", sidebarLayout(sidebarPanel(heatmapInput(ns("heatmap-clustering"), eselist, type = "samples"), width = 3), mainPanel(heatmapOutput(ns("heatmap-clustering"), 
            type = "samples"), width = 9)), icon = icon("th")), tabPanel("Feature-wise clustering", sidebarLayout(sidebarPanel(clusteringInput(ns("feature-clustering"), 
    ...
    if (any(unlist(lapply(eselist, function(ese) {
        ...
        exploratory_menu <- pushToList(exploratory_menu, tabPanel("Read reports", sidebarLayout(sidebarPanel(readreportsInput(ns("readrep"), eselist), 
            width = 3), mainPanel(readreportsOutput(ns("readrep")), width = 9)), icon = icon("bar-chart-o")))
    }
    ...
    if (length(eselist@contrasts) > 0) {
        ...
        differential_menu <- list("Differential", tabPanel("Tables", sidebarLayout(sidebarPanel(differentialtableInput(ns("differential"), eselist), 
            width = 3), mainPanel(differentialtableOutput(ns("differential")), width = 9)), icon = icon("table")), tabPanel("Fold change plots", sidebarLayout(sidebarPanel(foldchangeplotInput(ns("foldchange"), 
        ...
        if (any(unlist(lapply(eselist, function(ese) {
            ...
            differential_menu <- pushToList(differential_menu, tabPanel("Gene set barcode plots", value = "genesetbarcode", sidebarLayout(sidebarPanel(genesetbarcodeplotInput(ns("illuminaarray"), 
                eselist), width = 3), mainPanel(genesetbarcodeplotOutput(ns("illuminaarray")), width = 9)), icon = icon("barcode")))
